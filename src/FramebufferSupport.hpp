...
inline GLuint createTexture(int width, int height, GLenum internalFormat, GLenum format)
{
    GLuint texture = 0;
...
inline void requireComplete(const char* name)
{
...
inline void bindTexture(GLuint texture, GLenum textureUnit)
{
    glActiveTexture(textureUnit);
    glBindTexture(GL_TEXTURE_2D, texture);
}

inline void writePpm(const std::filesystem::path& path,
    int width,
    int height,
